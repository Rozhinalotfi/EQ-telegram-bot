...
> **Important:** Never push your live production `BOT_TOKEN` to a public GitHub repository. It is highly recommended to strip your token before committing, or manage sensitive strings via an environment file (`.env`) utilizing the `python-dotenv` package, ensuring `.env` is listed inside your `.gitignore`.
